...
interface UserDao {
    ...
    suspend fun addPointsAndLevelUp(userId: Int, points: Int) {
        addPoints(userId, points)
        
        // Ensure points don't go below zero
        val userAfterPoints = getUserById(userId)
        userAfterPoints?.let {
            if (it.totalPoints < 0) {
                updateUser(it.copy(totalPoints = 0))
            }
            
            val finalUser = getUserById(userId) ?: return
            val calculatedLevel = (finalUser.totalPoints / 1000) + 1
            if (calculatedLevel > finalUser.level) {
                updateUser(finalUser.copy(level = calculatedLevel))
            }
        }
    }
    ...
    /**
     * Update current onboarding step for resumption.
     * @param userId user ID
     * @param step step index (0-2)
     */
    @Query("UPDATE users SET onboarding_step = :step WHERE user_id = :userId")
