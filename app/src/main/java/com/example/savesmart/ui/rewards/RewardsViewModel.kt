...
class RewardsViewModel(private val repository: SaveSmartRepository) : ViewModel() {
    ...
    val earnedBadges: LiveData<List<Badge>> = _userId.switchMap { userId ->
        repository.getEarnedBadgesLive(userId)
    }
    ...
    /**
     * Convenience helper for tests and UI: set the active user and return earned badges LiveData.
     */
