...
object SecurityUtils {
    ...
    fun hashPassword(password: String): String {
        // Do not log any password or hash material — keep minimal logging for failures only
        return try {
            val digest = MessageDigest.getInstance(ALGORITHM)
            val hashBytes = digest.digest(password.toByteArray(Charsets.UTF_8))
            hashBytes.joinToString("") { byte -> "%02x".format(byte) }
        } catch (e: NoSuchAlgorithmException) {
            Log.e(TAG, "hashPassword() error — algorithm unavailable")
            ""
        }
    }
    ...
    fun verifyPassword(plainText: String, storedHash: String): Boolean {
        // Avoid logging verification attempts to prevent leaking sensitive info
        if (storedHash.isEmpty()) return false
        val inputHash = hashPassword(plainText)
        return inputHash.isNotEmpty() && inputHash == storedHash
    }
    ...
    fun isValidPassword(password: String): Boolean {
        // Simple strength check: min length + at least one letter + at least one digit
